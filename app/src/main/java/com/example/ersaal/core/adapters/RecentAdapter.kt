...
class RecentAdapter(
    ...
    private var dataList: ArrayList<RecentUsers>,
) :
    RecyclerView.Adapter<RecentAdapter.ViewHolder>() {
    inner class ViewHolder(val binding: ItemListRecentsBinding) :
        RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding =
            ItemListRecentsBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        with(viewHolder)
        {
            ...
            binding.tvFolder.text = itemAtPos.folder
        }
    }
